...
export default function ProjectsTab({ projects, currentWs, refetchProjects, workspaces }: ProjectsTabProps) {
  ...
  // Map real projects to the layout design of the screenshot, removing mockups
  const displayProjects = projects.map((p, idx) => {
    const statusOpts = ['IN_SYNC', 'IDLE', 'DEPLOYING', 'PAUSED'];
    const status = statusOpts[idx % statusOpts.length];
    
    const progressVal = Math.round(40 + (p.name.charCodeAt(0) % 55));
    
    const icons = [Cpu, Activity, Terminal, ShieldAlert];
    ...
    return {
      id: p.id,
      name: p.name,
      description: p.description || 'Microservice container and worker node.',
      status,
      metricLabel: status === 'DEPLOYING' ? 'DEPLOYING_TO_PROD' : status === 'IN_SYNC' ? 'COMPILATION_PROGRESS' : 'SYNC_COMPLETION',
      progress: progressVal,
      meta1Label: 'BUILD_ID',
      meta1Val: `#A92-FF${idx}`,
      meta2Label: 'LATENCY',
      meta2Val: `${10 + idx * 3}ms`,
      meta2Color: '#818cf8',
      meta3Label: 'UPTIME',
      meta3Val: '99.99%',
      meta3Color: '#0ea5e9',
      avatars: ['JD'],
      updateTime: `${idx + 1}m ago`,
      icon,
      gradient,
      real: true,
    };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          {displayProjects.length === 0 ? (
            <div style={{
              ...
              <Folder size={28} style={{ color: 'rgba(255,255,255,0.15)', marginBottom: '12px' }} />
              <h3 style={{ fontSize: '13.5px', fontWeight: 700, color: '#f5f5f7', margin: 0 }}>No active projects</h3>
              <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', margin: '4px 0 0 0' }}>
                Use the "+ New Project" button in the sidebar to create your first workspace project.
              </p>
            </div>
          ) : (
